...
"""
Backend Blocking Poll Pattern for Human-in-the-Loop Approvals (Async Version)

This is the async version of the backend blocking poll pattern, using `aiohttp` and
`asyncio.sleep()` for better concurrency in multi-agent systems.

## Benefits of Async Version

- ✅ Non-blocking: Doesn't tie up thread while waiting
- ✅ Better concurrency: Handles multiple approvals simultaneously
- ✅ Scalable: Suitable for high-throughput multi-agent systems
- ✅ Same simple API: Agent still calls tool once, receives final result

## When to Use

- High-concurrency multi-agent systems
- Multiple simultaneous approvals
- Long-running approval workflows (>5 minutes)
- Production deployments with scalability requirements

For simpler use cases or standalone agents, the sync version may be sufficient.
...
# Run this agent with async runner
...
async def request_approval_blocking_async(
    proposal: str,
    context: Optional[Dict[str, Any]] = None
) -> str:
    """
    Request human approval for a proposal via external API (async blocking poll pattern).

    This is the async version of request_approval_blocking, using aiohttp and asyncio
    for non-blocking I/O. Better for high-concurrency multi-agent systems.

    **Pattern**: Backend Blocking Poll (Async)
    - Tool polls internally using async I/O (agent doesn't see intermediate states)
    - Agent calls tool once, receives final decision
    - No manual "continue" clicks required
    - Doesn't block thread while waiting (better concurrency)
    ...
    Production Notes:
        ...
        - Uses async I/O - doesn't block thread while waiting
        - Better for high-concurrency scenarios than sync version
    ...
    try:
        logger.info("Creating approval ticket via external API (async)")
        ...
        async with aiohttp.ClientSession() as session:
            ...
            try:
                ...
                logger.error(f"Failed to create approval ticket: {e}", exc_info=True)
                return f"❌ Failed to create approval ticket: {str(e)}"
            ...
            # Step 2: Poll for approval decision (internal async loop - agent doesn't see this)
            elapsed_time = 0
            poll_count = 0
...
# Define the approval agent using the async blocking poll pattern
approval_agent_async = Agent(
    model='gemini-2.0-flash',
    name='approval_agent_async',
    description="Handles human-in-the-loop approval for proposals using async backend blocking poll pattern",
    instruction="""
    You are an Approval Agent that submits proposals to humans for review (async version).

    **Your Role**:
    Submit proposals for human approval and return the decision to the user.

    **How It Works**:
    1. When you receive a proposal that needs approval, call the `request_approval_blocking_async` tool
    2. The tool will:
       - Create an approval ticket in the external approval system
       - Poll the system every 30 seconds internally using async I/O (you won't see this)
       - Return the final decision when ready (or timeout after 10 minutes)
    3. Report the approval decision to the user

    **IMPORTANT**:
    - The approval tool BLOCKS until a decision is made (but uses async I/O for efficiency)
    ...
    **Example Interaction**:
    ...
    You: [Call request_approval_blocking_async with proposal details]
    ...
    **DO NOT**:
    ...
    """,
    tools=[request_approval_blocking_async],
...
if __name__ == "__main__":
    ...
    async def main():
        ...
        print("Testing async backend blocking poll pattern...")

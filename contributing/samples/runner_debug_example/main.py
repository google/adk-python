...
async def example_multiple_queries():
  """Debug with multiple queries in sequence."""
  print("\n------------------------------------")
...
async def example_conversation_persistence():
  ...
  # Fourth interaction - referring to previous queries
...
async def example_separate_sessions():
  ...
  await runner.run_debug(
      ...
      user_id="alice",
      session_name="alice_debug",
  )

  # Bob's session (separate)
  print("\n-- Bob's session --")
  await runner.run_debug(
      "Calculate 100 / 5", user_id="bob", session_name="bob_debug"
  )

...
from .service import CloudLoggingService
import logging
...
async def get_api_usage_analytics(
    ...
    Returns:
        Dictionary with API usage analytics including request counts,
        response times, popular endpoints, and user activity
    """
    try:
        cloud_logging_service: CloudLoggingService = req.app.state.cloud_logging_service
        if not cloud_logging_service:
            return {"success": False, "error": "CloudLoggingService not available"}
        
        result = cloud_logging_service.get_api_usage_analytics(project_id, hours)
        return result

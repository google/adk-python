...
def setup_environment():
    ...
    # Set Python path to the actual project root (parent of deploy folder)
    project_root = Path(__file__).parent.parent  # Go up one level from deploy/
...
def run_local():
    ...
    venv_python = str(venv_path / "bin" / "python") if (venv_path / "bin" / "python").exists() else sys.executable
    ...
    result = subprocess.run([pip_cmd, "install", "-q", "-r", str(requirements_file)], 
                          capture_output=True, text=True)
    if result.returncode != 0 and "already satisfied" not in result.stdout:
        print("Installing missing dependencies...")
        subprocess.run([pip_cmd, "install", "-q", "-r", str(requirements_file)])
...
"""
    ...
    # Run the server using virtual environment from project root
    os.chdir(str(project_root))  # Change to project root for proper imports
    subprocess.run([

...
from unified_api_client import api_client as simple_api
...
def render_dashboard_view():
    ...
    """
    # Get real asset data first for prominent display using unified client
    asset_data = simple_api.get_asset_summary(st.session_state.get('selected_project', 'mgm-digitalconcierge'))
...
def render_key_metrics_row():
    ...
    # Use unified API client (Single Source of Truth)
    with st.spinner("Loading asset inventory metrics..."):
        try:
            # Get metrics from unified client
            metrics = simple_api.get_metrics_for_dashboard(st.session_state.selected_project)
...
def render_recent_activity_section():
    ...
    # Use unified API client (DRY principle)
    with st.spinner("Loading asset discovery activity..."):
        try:
            # Get asset data from unified client
            asset_data = simple_api.get_asset_summary(st.session_state.selected_project)
    ...
    if asset_data.get('success') and asset_data.get('total_assets', 0) > 0:
        ...
        with col2:
            if st.button("🔄 Refresh Data", use_container_width=True):
                simple_api.clear_cache(st.session_state.get('selected_project'))
                st.rerun()
        st.markdown("---")

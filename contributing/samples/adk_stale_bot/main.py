...
async def main():
    ...
    prompt = "Find and process all open issues and PRs to manage staleness according to your rules."
    print(f"Agent Prompt: {prompt}\n")

    # The agent will now reason and use its tools to fulfill the request
    async for event in runner.run_async(user_id=USER_ID, session_id=session.id, new_message=prompt):

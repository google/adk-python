...
> `PERPLEXITY_API_KEY`, and pass `PerplexitySearchTool()` to `tools=`. Unlike
> `DiscoveryEngineSearchTool` (which requires a Discovery Engine app + data
> store), it works against the public web out of the box. See

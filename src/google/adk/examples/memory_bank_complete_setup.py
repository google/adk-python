...
from google.adk.utils.resource_utils import extract_agent_engine_id
from google.adk.utils.resource_utils import validate_agent_engine_resource_name
...
class MemoryBankSetup:
  ...
  def create_agent_engine(self):
    ...
    try:
      ...
      if validate_agent_engine_resource_name(resource_name):
        ...
        raise ValueError(
            f"Invalid Agent Engine resource name format: {resource_name}"
        )
  ...
  def setup_memory_service(self):
    ...
    if not self.agent_engine_id:
      raise ValueError(
          "Agent Engine ID not available. Call create_agent_engine() first."
      )
  ...
  def create_agent_with_memory(self):
    ...
    try:
      ...
      agent = adk.Agent(
          ...
          instruction=(
              "You are an AI assistant with persistent memory. Remember user"
              " preferences and conversation history."
          ),
          description=(
              "Agent with Memory Bank capabilities for persistent conversations"
          ),
...
def main():
  ...
  try:
    ...
    response = runner.run(
        "Hello! Please remember that my favorite color is blue."
    )

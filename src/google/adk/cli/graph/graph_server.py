...
<body>
  ...
  <script>
    ...
    $('add-node').onclick=()=>openAdd(); $('undo').onclick=undo; $('redo').onclick=redo; $('layout').onclick=()=>{ graphDocument.positions=advancedLayout(); render(); network.fit({animation:{duration:260,easingFunction:'easeInOutQuad'}}); markDirty('Smart layout applied'); toast('Grouped related agents and spaced nodes by label size.'); }; $('fit').onclick=()=>network.fit({animation:true}); $('zoom-in').onclick=()=>network.moveTo({scale:network.getScale()*1.15}); $('zoom-out').onclick=()=>network.moveTo({scale:network.getScale()*.85});

...
<body>
  ...
  <script>
    ...
    function validateDraft() { problemNodes=new Map(); const parents=new Map(topology.nodes.map(node=>[node.id,0])); const tools=new Map(topology.nodes.map(node=>[node.id,0])); topology.nodes.forEach(node=>{if(!node.label||!node.label.match(/^[A-Za-z_][A-Za-z0-9_]*$/)){problemNodes.set(node.id,'Use a valid Python identifier for this name.');} if(node.type==='tool'&&!node.config?.implementation&&!node.config?.read_only){problemNodes.set(node.id,'Add a Python implementation before generating source.');}}); topology.edges.forEach(edge=>{const source=topology.nodes.find(node=>node.id===edge.source),target=topology.nodes.find(node=>node.id===edge.target);if(!source||!target){return;}if(edge.type==='tool_binding'){tools.set(source.id,(tools.get(source.id)||0)+1);if(source.type!=='tool'||target.type!=='llm_agent')problemNodes.set(source.id,'Tools can only connect to LLM agents.');}else if(edge.type==='sub_agent'){parents.set(target.id,(parents.get(target.id)||0)+1);if(source.type==='tool'||target.type==='tool')problemNodes.set(target.id,'Sub-agent connections must join two agents.');}}); topology.nodes.forEach(node=>{if(node.type==='tool'&&(tools.get(node.id)||0)!==1)problemNodes.set(node.id,'Tool must be connected to exactly one LLM agent.');else if(node.type!=='tool'&&node.id!==topology.root_id&&(parents.get(node.id)||0)!==1)problemNodes.set(node.id,'Agent must have exactly one parent connection.');}); const messages=[...problemNodes.values()]; $('problems').hidden=!messages.length; $('problems').textContent=messages.length?`${messages.length} issue${messages.length===1?'':'s'} — ${messages[0]}`:''; return !messages.length; }

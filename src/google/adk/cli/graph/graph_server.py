...
<head>
  ...
  <style>
    ...
    * { box-sizing:border-box; }
    ...
    .hint { color:var(--muted); font-size:12px; margin:0 0 16px; } .palette { display:grid; grid-template-columns:repeat(2,minmax(0,1fr)); gap:8px; } .palette-heading { grid-column:1/-1; color:var(--muted); font-size:11px; font-weight:700; letter-spacing:.6px; margin:6px 0 0; text-transform:uppercase; }
    .palette-item { display:flex; align-items:center; gap:7px; width:100%; min-height:42px; padding:9px; text-align:left; border:1px solid var(--line); border-radius:9px; background:#fff; } .palette-item.agent { grid-column:1/-1; } .palette-item:hover { border-color:#a8c7fa; background:#f8fbff; box-shadow:0 1px 2px #00000014; }
    .type-dot { width:9px; height:9px; border-radius:3px; background:var(--type,#1a73e8); } .palette-item small { display:none; }
...
<body>
  ...
  <main id="workspace" class="workspace">
    <aside class="rail" aria-label="Node library"><div class="rail-inner"><div class="section-title">Build</div><p class="hint">Drag a card onto the canvas, or select it to add a node.</p><div id="palette" class="palette"></div><div class="divider"></div><button id="close-left" class="btn" aria-label="Collapse Library">‹ Library</button></div></aside>
    <section class="canvas-wrap" aria-label="Agent graph canvas"><div class="canvas-tools"><button id="add-node" class="btn primary">+ Add</button><button id="layout" class="btn">Auto layout</button><button id="undo" class="icon-btn" aria-label="Undo last graph change" data-tooltip="Undo (Ctrl/Cmd+Z)">↶</button><button id="redo" class="icon-btn" aria-label="Redo graph change" data-tooltip="Redo (Ctrl/Cmd+Shift+Z)">↷</button><button id="zoom-out" class="icon-btn" aria-label="Zoom out" data-tooltip="Zoom out">−</button><span id="zoom-level" class="status">100%</span><button id="zoom-in" class="icon-btn" aria-label="Zoom in" data-tooltip="Zoom in">+</button><button id="fit" class="btn">Fit</button><button id="toggle-left" class="btn" aria-label="Toggle Library">Library ›</button><button id="toggle-right" class="btn" aria-label="Toggle Inspector">‹ Inspector</button></div><div id="network"></div><div id="empty" class="empty"><div class="empty-card"><h2>Start building your agent system</h2><p>Add an agent or tool from the library. Your source code stays tucked away until you open the Code drawer.</p></div></div></section>
    <aside class="inspector" aria-label="Inspector"><div class="inspector-inner"><div class="section-title">Inspector</div><div id="problems" class="hint" hidden></div><div id="inspector-empty" class="hint">Click an agent on the canvas to edit its name, model, instruction, and tools here.</div><form id="node-form" hidden><h2 id="node-heading" class="node-title"></h2><p id="node-type" class="node-meta"></p><div class="field"><label for="node-label">Name</label><input id="node-label" required></div><div class="field"><label for="node-description">Description</label><textarea id="node-description"></textarea></div><div class="field"><label for="node-kind">Node type</label><select id="node-kind"><option value="llm_agent">LLM agent</option><option value="sequential">Sequential workflow</option><option value="parallel">Parallel workflow</option><option value="loop">Loop workflow</option><option value="tool">Tool</option></select></div><div class="field config-agent"><label for="node-model">Model</label><input id="node-model" placeholder="gemini-2.5-flash"></div><div class="field config-agent"><label for="node-instruction">Instruction</label><textarea id="node-instruction"></textarea></div><div class="field config-loop" hidden><label for="node-max-iterations">Maximum iterations</label><input id="node-max-iterations" type="number" min="1"></div><div class="field config-tool" hidden><label for="tool-implementation">Python implementation</label><textarea id="tool-implementation" spellcheck="false"></textarea></div><button class="btn primary" type="submit">Apply changes</button><button id="delete-node" class="btn" type="button">Delete node</button></form><div id="edge-inspector" hidden><h2 class="node-title">Connection</h2><p id="edge-type" class="node-meta"></p><p id="edge-endpoints" class="hint"></p><button id="reconnect-edge" class="btn" type="button">Reconnect</button><button id="delete-edge" class="btn" type="button">Delete connection</button></div><div class="divider"></div><button id="close-right" class="btn" aria-label="Collapse Inspector">Inspector ›</button></div></aside>
  </main>
  ...
  <script>
    ...
    function toolsFor(node) { return topology.edges.filter(edge=>edge.type==='tool_binding'&&edge.target===node.id).map(edge=>topology.nodes.find(item=>item.id===edge.source)?.label).filter(Boolean); }
    function nodeView(node) { const [typeName,color] = TYPES[node.type] || ['Component','#5f6368']; const summary=(node.description||node.config?.instruction||'').replace(/\s+/g,' ').trim(); const isFlow=['sequential','parallel','loop'].includes(node.type); const icon=isFlow?({sequential:'⇢',parallel:'⋔',loop:'↻'}[node.type]):'◉'; const tools=toolsFor(node); const lines=isFlow?[`${icon} ${typeName.toUpperCase()}`,node.label,`${node.sub_agents?.length||0} ${node.type==='parallel'?'branches':'steps'}`]:[`🤖 ${node.label}`,typeName,...wrapText(summary,32).slice(0,2),tools.length?`🔧 ${tools.join('  ·  ')}`:'']; const visibleLines=lines.filter(Boolean); const longest=Math.max(...visibleLines.map(line=>line.length),14); const problem=problemNodes.get(node.id); return { id:node.id, label:visibleLines.join('\n'), title:problem||'', width:Math.max(isFlow?220:190,Math.min(330,longest*7+42)), shape:'box', margin:{top:isFlow?12:16,right:18,bottom:16,left:18}, color:{background:isFlow?'#f7f2fc':'#ffffff',border:problem?'#d93025':color,highlight:{background:'#e8f0fe',border:'#1a73e8'}}, borderWidth:problem?3:2, chosen:{node:(values)=>{values.shadow=true;values.shadowColor='#1a73e844';}}, font:{face:'Arial',color:'#202124',size:14,align:'center'}, shadow:true }; }
    function edgeView(edge) { return { id:edge.id, from:edge.source, to:edge.target, arrows:'to', color:{color:'#7b8086',highlight:'#1a73e8'}, width:2, smooth:{type:'horizontal',roundness:.12}, font:{color:'#3c4043',size:12,align:'horizontal',strokeWidth:4,strokeColor:'#f8fafd'}, label:'' }; }
    function validateDraft() { problemNodes=new Map(); const byId=new Map(),parents=new Map(),tools=new Map(),adjacency=new Map(); topology.nodes.forEach(node=>{if(byId.has(node.id))problemNodes.set(node.id,'Each graph node must have a unique id.');byId.set(node.id,node);parents.set(node.id,0);tools.set(node.id,0);adjacency.set(node.id,[]);if(!node.label||!node.label.match(/^[A-Za-z_][A-Za-z0-9_]*$/))problemNodes.set(node.id,'Use a valid Python identifier for this name.');if(node.type==='tool'&&!node.config?.implementation&&!node.config?.read_only)problemNodes.set(node.id,'Add a Python implementation before generating source.');}); if(topology.nodes.length&&!byId.has(topology.root_id))problemNodes.set(topology.nodes[0].id,'Select a valid root agent.'); topology.edges.forEach(edge=>{const source=byId.get(edge.source),target=byId.get(edge.target);if(!source||!target){problemNodes.set(source?.id||target?.id||topology.root_id,'Connection references a missing node.');return;}if(edge.type==='tool_binding'){tools.set(source.id,(tools.get(source.id)||0)+1);if(source.type!=='tool'||target.type!=='llm_agent')problemNodes.set(source.id,'Tools can only connect to LLM agents.');}else if(edge.type==='sub_agent'){parents.set(target.id,(parents.get(target.id)||0)+1);adjacency.get(source.id).push(target.id);if(source.type==='tool'||target.type==='tool')problemNodes.set(target.id,'Sub-agent connections must join two agents.');}else problemNodes.set(source.id,'Unsupported connection type.');}); const visiting=new Set(),visited=new Set(); const visit=id=>{if(visiting.has(id)){problemNodes.set(id,'Sub-agent connections cannot contain a cycle.');return;}if(visited.has(id))return;visiting.add(id);adjacency.get(id)?.forEach(visit);visiting.delete(id);visited.add(id);}; adjacency.forEach((_,id)=>visit(id)); topology.nodes.forEach(node=>{if(node.type==='tool'&&(tools.get(node.id)||0)!==1)problemNodes.set(node.id,'Tool must be connected to exactly one LLM agent.');else if(node.type!=='tool'&&node.id!==topology.root_id&&(parents.get(node.id)||0)!==1)problemNodes.set(node.id,'Agent must have exactly one parent connection.');}); const messages=[...new Set(problemNodes.values())]; $('problems').hidden=!messages.length; $('problems').textContent=messages.length?`${messages.length} issue${messages.length===1?'':'s'} — ${messages[0]}`:''; return !messages.length; }
    ...
    function clearGraphFocus() { topology.nodes.filter(node=>node.type!=='tool').forEach(node=>nodes.update(positionedNode(node))); topology.edges.filter(edge=>edge.type!=='tool_binding').forEach(edge=>edges.update(edgeView(edge))); }
    function highlightRelatedNodes(id) { const node=topology.nodes.find(item=>item.id===id); if(!node)return; clearGraphFocus(); const highlighted=new Set([id]); const highlightedEdges=new Set(); topology.edges.forEach(edge=>{if(edge.source===id||edge.target===id){highlighted.add(edge.source);highlighted.add(edge.target);highlightedEdges.add(edge.id);}}); topology.nodes.filter(item=>item.type!=='tool').forEach(item=>{const view=positionedNode(item);const isMatch=highlighted.has(item.id);nodes.update({...view,color:{background:isMatch?'#e8f0fe':'#f8f9fa',border:isMatch?'#1a73e8':'#dadce0'},borderWidth:isMatch?4:1,font:{...view.font,color:isMatch?'#202124':'#9aa0a6'},shadow:isMatch});}); topology.edges.filter(edge=>edge.type!=='tool_binding').forEach(edge=>{const view=edgeView(edge);edges.update({...view,color:{...view.color,color:highlightedEdges.has(edge.id)?'#1a73e8':'#d3d6da'},width:highlightedEdges.has(edge.id)?3:1});}); }
    function render() { validateDraft(); const visibleNodes=topology.nodes.filter(node=>node.type!=='tool'); const visibleEdges=topology.edges.filter(edge=>edge.type!=='tool_binding'); nodes = new vis.DataSet(visibleNodes.map(positionedNode)); edges = new vis.DataSet(visibleEdges.map(edgeView)); if (network) network.destroy(); const hasPositions=Object.keys(graphDocument.positions).length>0; network = new vis.Network($('network'), {nodes,edges}, { physics:false, layout:{hierarchical:hasPositions?false:graphLayout()}, interaction:{dragNodes:true,dragView:true,hover:true,multiselect:true,navigationButtons:false}, manipulation:{enabled:false,addEdge:(data,callback)=>{if(data.from===data.to){callback(null);return;}const edge=topology.edges.find(item=>item.id===reconnectingEdge);if(!edge){callback(null);return;}Object.assign(edge,{source:data.from,target:data.to,type:'sub_agent'});reconnectingEdge=null;callback(null);render();markDirty('Connection updated');}} }); network.on('selectNode', params => inspectNode(params.nodes[0])); network.on('selectEdge', params => inspectEdge(params.edges[0])); network.on('deselectNode', () => {clearGraphFocus();clearInspector();}); network.on('dragEnd', params => { capturePositions(params.nodes); markDirty('Layout changed'); }); network.on('zoom',()=>{$('zoom-level').textContent=`${Math.round(network.getScale()*100)}%`;}); $('empty').style.display=visibleNodes.length?'none':'grid'; }
    function populatePalette() { const groups=[['Agents',['llm_agent']],['Flow',['sequential','parallel','loop']],['Capabilities',['tool']]]; $('palette').innerHTML=groups.map(([title,types])=>`<div class="palette-heading">${title}</div>${types.map(type=>{const [name,color]=TYPES[type];return `<button class="palette-item ${type==='llm_agent'?'agent':''}" draggable="true" data-type="${type}" style="--type:${color}"><span class="type-dot"></span><strong>${name}</strong></button>`;}).join('')}`).join(''); document.querySelectorAll('.palette-item').forEach(item=>{ item.addEventListener('click',()=>openAdd(item.dataset.type)); item.addEventListener('dragstart', event=>event.dataTransfer.setData('text/plain',item.dataset.type)); }); }
    function updateNewNodeFields() { $('new-tool-field').hidden=$('new-type').value!=='tool'; }
    function openAdd(type='llm_agent') { $('new-type').value=type; $('new-name').value=''; $('new-description').value=''; $('new-tool-implementation').value=''; updateNewNodeFields(); $('modal-backdrop').classList.add('open'); $('new-name').focus(); }
    ...
    $('node-modal').addEventListener('submit', event=>{ event.preventDefault(); const label=$('new-name').value.trim(),type=$('new-type').value; if(!label) return; const id=label.replace(/[^A-Za-z0-9_]/g,'_')+'_'+Date.now(); const selectedParent=selectedNode()||topology.nodes.find(node=>node.id===topology.root_id); if(type==='tool'&&selectedParent?.type!=='llm_agent'){toast('Select an LLM agent before adding a tool.');return;} if(type!=='tool'&&topology.root_id&&!selectedParent){toast('Select a parent agent before adding a node.');return;} const config=type==='tool'?{implementation:$('new-tool-implementation').value}:{}; topology.nodes.push({id,type,label,description:$('new-description').value.trim(),sub_agents:[],tools:[],parent_id:selectedParent?.id||null,config}); if(!topology.root_id)topology.root_id=id; else if(type==='tool')topology.edges.push({id:'edge_tool_'+id+'_'+selectedParent.id,source:id,target:selectedParent.id,type:'tool_binding',label:null}); else topology.edges.push({id:'edge_sub_'+selectedParent.id+'_'+id,source:selectedParent.id,target:id,type:'sub_agent',label:null}); closeAdd(); render(); if(type==='tool'){network.selectNodes([selectedParent.id]);toast(`Tool attached to ${selectedParent.label}.`);}else network.selectNodes([id]); markDirty('Node added'); });
    $('node-form').addEventListener('submit', event=>{ event.preventDefault(); const node=selectedNode(); if(!node) return; node.label=$('node-label').value.trim()||node.label; node.description=$('node-description').value.trim(); node.type=$('node-kind').value; node.config=node.config||{}; node.config.model=$('node-model').value.trim(); node.config.instruction=$('node-instruction').value.trim(); const iterations=Number($('node-max-iterations').value); if(node.type==='loop'&&iterations>0)node.config.max_iterations=iterations;else delete node.config.max_iterations; if(node.type==='tool')node.config.implementation=$('tool-implementation').value; render(); network.selectNodes([node.id]); markDirty('Node updated'); });
    ...
    $('delete-edge').onclick=()=>{ topology.edges=topology.edges.filter(edge=>edge.id!==selected); clearInspector(); render(); markDirty('Connection deleted'); }; $('reconnect-edge').onclick=()=>{reconnectingEdge=selected;network.addEdgeMode();toast('Drag from the new parent to the new child.');};
    ...
    $('new-type').onchange=updateNewNodeFields; $('add-node').onclick=()=>openAdd(); $('undo').onclick=undo; $('redo').onclick=redo; $('layout').onclick=()=>{ graphDocument.positions=advancedLayout(); render(); network.fit({animation:{duration:260,easingFunction:'easeInOutQuad'}}); markDirty('Smart layout applied'); toast('Grouped related agents and spaced nodes by label size.'); }; $('fit').onclick=()=>network.fit({animation:true}); $('zoom-in').onclick=()=>network.moveTo({scale:network.getScale()*1.15}); $('zoom-out').onclick=()=>network.moveTo({scale:network.getScale()*.85});

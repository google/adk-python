...
<body>
  ...
  <script>
    ...
    function render() { validateDraft(); nodes = new vis.DataSet(topology.nodes.map(positionedNode)); edges = new vis.DataSet(topology.edges.map(edgeView)); if (network) network.destroy(); const hasPositions=Object.keys(graphDocument.positions).length>0; network = new vis.Network($('network'), {nodes,edges}, { physics:false, layout:{hierarchical:hasPositions?false:graphLayout()}, interaction:{dragNodes:true,dragView:true,hover:true,multiselect:true,navigationButtons:false}, manipulation:{enabled:false,addEdge:(data,callback)=>{ if(data.from===data.to){toast('A node cannot connect to itself.'); callback(null);return;} const source=topology.nodes.find(n=>n.id===data.from),target=topology.nodes.find(n=>n.id===data.to); const type=source && source.type==='tool' ? 'tool_binding' : 'sub_agent'; if((type==='tool_binding'&&target?.type!=='llm_agent')||(type==='sub_agent'&&target?.type==='tool')){toast('That connection is not valid for these node types.');callback(null);return;} if(reconnectingEdge){const edge=topology.edges.find(item=>item.id===reconnectingEdge);Object.assign(edge,{source:data.from,target:data.to,type});reconnectingEdge=null;}else{topology.edges.push({id:'edge_'+Date.now(),source:data.from,target:data.to,type,label:null});} callback(null); $('connect').classList.remove('active'); render(); markDirty('Connection updated'); }} }); network.on('selectNode', params => inspectNode(params.nodes[0])); network.on('selectEdge', params => inspectEdge(params.edges[0])); network.on('deselectNode', () => {clearGraphFocus();clearInspector();}); network.on('dragEnd', params => { capturePositions(params.nodes); markDirty('Layout changed'); }); $('empty').style.display=topology.nodes.length?'none':'grid'; }
    ...
    $('delete-edge').onclick=()=>{ topology.edges=topology.edges.filter(edge=>edge.id!==selected); clearInspector(); render(); markDirty('Connection deleted'); }; $('reconnect-edge').onclick=()=>{reconnectingEdge=selected;network.addEdgeMode();$('connect').classList.add('active');toast('Drag the connection from its new source to its new target.');};

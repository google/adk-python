...
class BaseAgent(BaseModel):
  ...
  Returns:
    Optional[types.Content]: The content to return to the user.
      When present, the actual model response
      will be ignored and the provided content will be returned to user.
  """

  single_turn_timeout: Optional[float] = None
  """Timeout for a single LLM call in seconds.

  If set, each individual LLM call (including streaming responses) will be
  canceled if it takes longer than this value. None means no timeout.

  This timeout is independent of `total_timeout`. If both are set, whichever
  triggers first will raise an AgentTimeoutError.
  """

  total_timeout: Optional[float] = None
  """Total timeout for the entire agent run in seconds.

  If set, the entire agent execution (including all LLM calls, tool calls,
  and sub-agent executions) will be canceled if it takes longer than this
  value. None means no timeout.

  This timeout is independent of `single_turn_timeout`. If both are set,
  whichever triggers first will raise an AgentTimeoutError.
  """
  ...
  #
  # The timeout implementation in run_async() and run_live() uses a
  # queue + background task pattern instead of Python 3.11's asyncio.timeout()
  # context manager for two reasons:
  #
  # 1. Python 3.10 support: asyncio.timeout() was added in Python 3.11,
  #    but ADK requires Python 3.10+ compatibility.
  #
  # 2. AsyncGenerator limitation: asyncio.wait_for() cannot be used directly
  #    with async generators (functions that use 'yield'). The queue + task
  #    pattern wraps the async generator execution in a background task,
  #    allowing timeout control via asyncio.wait_for() on queue.get().
  #
  # How it works:
  # - A background task (_run_inner) executes the actual agent logic and
  #   sends events/errors to a queue.
  # - The main loop consumes from the queue using asyncio.wait_for() with
  #   a decreasing timeout.
  # - On timeout, the background task is cancelled, which cascades to any
  #   sub-agents via asyncio.CancelledError.
  # ============================================================================
  ...
  async def run_async(
    ...
    queue: asyncio.Queue[
        tuple[str, Event | Exception | None]
    ] = asyncio.Queue()
    done = asyncio.Event()
      ...
      while True:
        try:
          ...
          if self.total_timeout is not None:
            ...
            remaining_timeout = max(0.0, self.total_timeout - elapsed)
          ...
          raise AgentTimeoutError(
              ...
              trigger=TimeoutTrigger.USER_INPUT,
              agent_name=self.name,
          )

        if item[0] == 'event':
          event = cast(Event, item[1])
          yield event
        elif item[0] == 'error':

...
class FirestoreSessionService(BaseSessionService):
  ...
  async def delete_session(
    ...
    def _delete_in_firestore():
      ...
      if (
          ...
          or session_doc.to_dict().get("user_id") != user_id
          or session_doc.to_dict().get("app_name") != app_name
      ):
        return
  ...
  async def update_session_state(
      self, session_id: str, state_delta: Dict[str, Any]
  ):
    ...
    def _update_in_firestore():
      ...
      update_data = {
          f"state.{key}": value for key, value in state_delta.items()
      }
  ...
  async def append_event(self, session: Session, event: Event) -> Event:
    ...
    def _append_in_firestore():
      ...
      try:
        ...
        # Commit the batch.
        logger.info(
            "Committing batch to Firestore for session '%s'...", session.id
        )
        batch.commit()
        logger.info(
            "Batch committed successfully for session '%s'.", session.id
        )
  ...
  if event.content:
    ...
    if event.author == "user" and "parts" in content_dict:
      ...
      for part in content_dict["parts"]:
        ...
        if (
            "inline_data" in part
            and part["inline_data"].get("mime_type") == "application/pdf"
        ):

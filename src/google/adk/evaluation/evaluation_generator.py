...
class _LiveSession:
  ...
  async def _consume_events(self) -> None:
    ...
    try:
      ...
      if not isinstance(self.runner.agent, Agent):
        await self._consume_node_events()
        return

      run_config = self._live_run_config()
  ...
  @staticmethod
  def _live_run_config() -> RunConfig:
    """Builds the live run config shared by the live drivers."""
    return RunConfig(
  ...
  def _is_normal_closure(exc: BaseException) -> bool:
    ...
    return isinstance(exc, ConnectionClosedOK) or (
        isinstance(exc, errors.APIError) and exc.code == 1000
    )

...
"""Active streaming tool management for Live sessions."""
...
class ActiveStreamingTool(BaseModel):
  """Manages streaming tool related resources during live invocation."""
  ...
  def model_post_init(self, __context: Any) -> None:
    """Adds a task supplied through the compatibility fields."""
    del __context
    if self.task is not None:
      self._track_task(self.task, self.stream)
  ...
  def _track_task(
    ...
    self.task = task
    self.stream = stream
    if task not in self._task_streams:

...
def _is_transcription(event: Event) -> bool:
    return (
        event.input_transcription is not None or event.output_transcription is not None
    )


def _has_non_empty_transcription_text(transcription) -> bool:
    return bool(transcription and transcription.text and transcription.text.strip())
...
class Runner:
    ...
    def _validate_runner_params(
        ...
        if app:
            ...
            raise ValueError("Either app or both app_name and agent must be provided.")
        else:
    ...
    def _should_append_event(self, event: Event, is_live_call: bool) -> bool:
        ...
        if is_live_call and contents._is_live_model_audio_event_with_inline_data(event):
                    ...
                    if is_live_call:
                        ...
                        if event.partial is not True:
                            if _is_transcription(event) and (
                                _has_non_empty_transcription_text(
                                ...
                                for buffered_event in buffered_events:
                                    logger.debug(
                                        "Appending buffered event: %s", buffered_event
                                    )
                                    await self.session_service.append_event(
                                ...
                                if self._should_append_event(event, is_live_call):
                                    logger.debug(
                                        "Appending non-buffered event: %s", event
                                    )
                                    await self.session_service.append_event(
                                        session=session, event=event
                                    )

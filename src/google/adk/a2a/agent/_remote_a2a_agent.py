...
from ..logs.log_utils import build_a2a_request_log
...
from .interceptors.new_integration_extension import _NEW_A2A_ADK_INTEGRATION_EXTENSION
from .interceptors.new_integration_extension import _new_integration_extension_interceptor
from .utils import execute_after_request_interceptors
from .utils import execute_before_card_request_interceptors
from .utils import execute_before_request_interceptors
  ...
  async def _ensure_resolved(
    ...
    try:
      ...
      if not self._a2a_client:
        ...
        if not self._a2a_client_factory:
          raise AgentCardResolutionError(
              f"Failed to initialize A2A client factory for {self.name}"
          )
        self._a2a_client = self._a2a_client_factory.create(self._agent_card)

      if not self._a2a_client:
        raise AgentCardResolutionError(
            f"Failed to initialize A2A client for {self.name}"
        )

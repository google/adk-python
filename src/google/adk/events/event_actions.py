...
class EventActions(BaseModel):
  ...
  """A list of `ApprovalRequest` objects generated by tools during their execution.

  When a tool determines that it requires specific approvals to proceed (e.g., due to
  its internal logic or policies it's aware of, though typically policies are checked
  by `ApprovalHandler` before tool execution), it can populate this list.
  The `ApprovalHandler` or a flow can then use these requests to initiate an
  approval workflow with the user.
  This is primarily used when a tool call is made, and the `ApprovalHandler` determines

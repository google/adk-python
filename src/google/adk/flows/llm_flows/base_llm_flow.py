...
class BaseLlmFlow(ABC):
        ...
        while True:
            try:
                ...
                if invocation_context.live_session_resumption_handle:
                    ...
                    llm_request.live_connect_config.session_resumption.transparent = (
                        True
                    )
                        ...
                        async with Aclosing(
                            ...
                            async for event in agen:
                                ...
                                if (
                                    ...
                                    and event.content.parts[
                                        0
                                    ].inline_data.mime_type.startswith("audio/")
                                ):
                                    audio_blob = types.Blob(
                                        data=event.content.parts[0].inline_data.data,
                                        mime_type=event.content.parts[
                                            0
    ...
    async def _preprocess_async(
        ...
        if not isinstance(agent, LlmAgent):
            raise TypeError(f"Expected agent to be an LlmAgent, but got {type(agent)}")
    ...
    async def _call_llm_async(
        ...
        async def _call_llm_body() -> AsyncGenerator[LlmResponse, None]:
        ...
        async with Aclosing(_call_llm_with_optional_tracing()) as agen:
            async for event in agen:
                yield event

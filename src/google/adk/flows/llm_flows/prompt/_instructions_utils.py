...
"""Utilities for instruction templates and session state injection."""
...
async def inject_session_state(
  ...
  ```
  ...
  from google.adk.flows.llm_flows.prompt import inject_session_state
...
async def _render_with_regex(
  ...
  async def _async_sub(
      ...
      repl_async_fn: Callable[[re.Match[str]], Awaitable[str]],
      string: str,
  ) -> str:
    result = []
    last_end = 0
  ...
  async def _replace_match(match: re.Match[str]) -> str:

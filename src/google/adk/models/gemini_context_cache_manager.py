...
class GeminiContextCacheManager:
    ...
    async def handle_context_caching(self, llm_request: LlmRequest) -> Optional[CacheMetadata]:
        ...
        if llm_request.cache_metadata:
            ...
            if await self._is_cache_valid(llm_request):
                ...
                current_fingerprint = self._generate_cache_fingerprint(llm_request, cache_contents_count)
                ...
                if current_fingerprint == old_cache_metadata.fingerprint:
                    ...
                    cache_metadata = await self._create_new_cache_with_contents(llm_request, cache_contents_count)
                    if cache_metadata:
                        self._apply_cache_to_request(llm_request, cache_metadata.cache_name, cache_contents_count)
                        return cache_metadata

                # Fingerprints don't match - recalculate with cacheable contents only
                logger.debug("Fingerprints don't match, returning fingerprint-only metadata")
                cacheable_count = self._find_count_of_contents_to_cache(llm_request.contents)
                fingerprint_for_all = self._generate_cache_fingerprint(llm_request, cacheable_count)
                return CacheMetadata(
                    fingerprint=fingerprint_for_all,
                    contents_count=cacheable_count,
                )
        ...
        cacheable_count = self._find_count_of_contents_to_cache(llm_request.contents)
        fingerprint = self._generate_cache_fingerprint(llm_request, cacheable_count)
        return CacheMetadata(
    ...
    async def _is_cache_valid(self, llm_request: LlmRequest) -> bool:
        ...
        if cache_metadata.invocations_used > llm_request.cache_config.cache_intervals:
            logger.info(
    ...
    def _generate_cache_fingerprint(self, llm_request: LlmRequest, cache_contents_count: int) -> str:
        ...
        if llm_request.config and llm_request.config.system_instruction:
            fingerprint_data["system_instruction"] = llm_request.config.system_instruction
        ...
        if llm_request.config and llm_request.config.tool_config:
            fingerprint_data["tool_config"] = llm_request.config.tool_config.model_dump()
    ...
    async def _create_new_cache_with_contents(
        ...
        if llm_request.cacheable_contents_token_count < llm_request.cache_config.min_tokens:
            logger.info(
    ...
    async def _create_gemini_cache(self, llm_request: LlmRequest, cache_contents_count: int) -> CacheMetadata:
        ...
        with tracer.start_as_current_span("create_cache") as span:
            ...
            cache_config = types.CreateCachedContentConfig(
                ...
                display_name=(f"adk-cache-{int(time.time())}-{cache_contents_count}contents"),
            )
            ...
            return CacheMetadata(
                ...
                fingerprint=self._generate_cache_fingerprint(llm_request, cache_contents_count),
                invocations_used=1,
    ...
    def populate_cache_metadata_in_response(self, llm_response: LlmResponse, cache_metadata: CacheMetadata) -> None:

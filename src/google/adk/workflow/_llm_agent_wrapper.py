...
def prepare_llm_agent_context(agent: Any, ctx: Context) -> Context:
  ...
  ic.session = ic.session.model_copy(deep=True)  # Deep copy to avoid shared history
  ic.session.events = []  # Start with a clean event list for single_turn agents
  return agent_ctx
...
def prepare_llm_agent_input(agent: Any, ctx: Context, node_input: Any) -> None:
  ...
  # Explicitly set the branch on injected user input to match child agent
...
async def run_llm_agent_as_node(
  ...
  if agent.mode == 'chat':
    ...
    #   2. Run parent.run_async: LLM emits a fresh task FC -
    #      dispatch and synthesize FR.

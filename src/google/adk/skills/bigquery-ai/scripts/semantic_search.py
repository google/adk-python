...
def generate_search_sql(
    ...
    if use_index:
        options = f", options => '{{\"fraction_lists_to_search\": {fraction_lists}}}'"
    else:
...
def main():
    ...
    embedding_model = args.embedding_model or f"{args.project_id}.{dataset}.text_embedding_model"
    ...
    try:
        ...
        print(f"Searching for: {args.query[:50] if args.query else 'batch queries'}...")
        ...
        if args.output == "json":
            ...
            for i, row in enumerate(results, 1):
                ...
                print(f"{i:<6}{str(row.id)[:18]:<20}{row.similarity_score:.4f}      {content_preview}")

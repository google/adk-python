...
def mock_k8s_config():
    ...
    with patch("google.adk.code_executors.gke_code_executor.config") as mock_config:
...
class TestGkeCodeExecutor:
    ...
    def test_execute_code_success(
        ...
        mock_k8s_clients["core_v1"].read_namespaced_pod_log.return_value = "hello world"
    ...
    def test_execute_code_job_failed(
        ...
        executor = GkeCodeExecutor()
        result = executor.execute_code(mock_invocation_context, CodeExecutionInput(code="fail"))
    ...
    def test_execute_code_api_exception(
        ...
        executor = GkeCodeExecutor()
        result = executor.execute_code(mock_invocation_context, CodeExecutionInput(code="..."))
    ...
    def test_execute_code_timeout(
        ...
        mock_watch.return_value.stream.return_value = []  # Empty stream simulates timeout
        mock_k8s_clients["core_v1"].read_namespaced_pod_log.return_value = (
            "Still running..."
        )
    ...
    def test_create_job_manifest_structure(self, mock_invocation_context):
        ...
        job = executor._create_job_manifest("test-job", "test-cm", mock_invocation_context)

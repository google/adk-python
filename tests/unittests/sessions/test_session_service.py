...
def test_database_session_service_strips_timezone_for_dialect(dialect_name):
  ...
  requires naive datetimes. MySQL's DATETIME type silently drops timezone
  info, causing stale-writer false positives when the marker string differs
  between tz-aware writes and naive reads.
  """
  ...
  # SQLite, PostgreSQL, and MySQL should have timezone stripped
...
def test_database_session_service_preserves_timezone_for_other_dialects():
  ...
  # For dialects like Spanner with explicit timezone support, we don't strip
  ...
  # Spanner should preserve timezone
  assert now.tzinfo is not None

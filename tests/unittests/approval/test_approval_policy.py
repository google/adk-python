...
def test_tool_policy__agent_tool():
  agent = Agent(name="useful_agent", model=utils.MockModel.create(responses=[]))
...
def test_tool_policy__langchain_tool(tmp_path):
  ...
  langchain_tool = LangchainTool(gitlab_get_mr_for_repo, policies=[
    FunctionToolPolicy(
      actions=[GIT_READ],
...
def test_tool_policy__crewai_tool(tmp_path):
  ...
  crewai_tool = CrewaiTool(gitlab_get_mr_for_repo, name="", description="", policies=[
    FunctionToolPolicy(
      actions=[GIT_READ],
      resource_mappers=lambda args: [
          f"{GIT_NAMESPACE}:{args['repo_path']}",

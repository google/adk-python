...
async def test_request_processor__no_approvals():
  ...
  session = runner.runner.session_service.get_session(
  ...
  assert (
      invocation_context.session.state.get("approvals", {}).get("grants", [])
      == []
  )
  invocation_context.session.state["approvals"] = {
      "grants": [],
      "suspended_function_calls": [function_call_to_approve],
  }
  events = [
      event
  ...
  assert state["approvals"]["grants"] == [grant.model_dump(mode="json")]
  ...
  assert state["approvals"]["suspended_function_calls"] == []

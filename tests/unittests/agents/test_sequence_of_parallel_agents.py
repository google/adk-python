...
def test_sequential_parallels():
  ...
  david_contents = testing_utils.simplify_contents(
      david_model.requests[0].contents
  )
  ...
  assert (
      "Alice" in david_text or "I am Alice" in david_text
  ), f"David should see Alice's output. Got: {david_text[:200]}"
  assert (
      "Bob" in david_text or "I am Bob" in david_text
  ), f"David should see Bob's output. Got: {david_text[:200]}"

  print(
      "\n✅ SUCCESS! David can see Alice and Bob (common prefix filtering"
      " works!)"
  )

...
# ---------------------------------------------------------------------------
# Regression coverage for the A2A human-input resume rewrite and its
# adversarial follow-ups: credential egress via the caller fallback, and a
# parallel real-tool + human-input resume re-creating the ValueError.
# ---------------------------------------------------------------------------

_SECRET = "not-a-real-access-token-0123456789"
...
def _resume_events(
  ...
  Args:
    ...
    responses: list of ``(name, id, response_dict)`` user function responses.
      More than one function_response can be placed on the resume event, which
      is required to reproduce the parallel real-tool + human-input case.
...
def _make_ctx(events):
  ...
  # The before-request interceptor hook seeds a ClientCallContext from the
  # session state, so it has to be a real mapping.
  ctx.session.state = {}
...
class TestHitlResumeRewrite:
  ...
  A workflow that pauses on a RequestInput node and then invokes an A2A
  ...
  async def test_run_async_impl_never_forwards_credential_to_peer(self):
    ...
    assert captured, "the peer was never sent a request"

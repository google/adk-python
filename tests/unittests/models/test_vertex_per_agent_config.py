...
def test_claude_custom_config():
    claude = Claude(
        project_id="test-project-claude",
...
def test_gemini_custom_config():
    gemini = Gemini(
        project_id="test-project-gemini",
...
@patch.dict('os.environ', {'GOOGLE_CLOUD_PROJECT': 'env-project', 'GOOGLE_CLOUD_LOCATION': 'env-location'})
...
def test_mixed_configuration():
    ...
    key_custom = f"{claude_custom.project_id or 'default'}:{claude_custom.location or 'default'}"
    key_default = f"{claude_default.project_id or 'default'}:{claude_default.location or 'default'}"

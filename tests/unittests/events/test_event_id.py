...
def test_event_id_is_uuid_format():
    ...
    assert uuid_pattern.match(event.id), f"Event ID {event.id} is not a valid UUID4"
...
def test_event_id_uniqueness_sequential():
    ...
    assert len(unique_ids) == len(event_ids), f"Found {len(event_ids) - len(unique_ids)} duplicate IDs"
...
def test_event_id_uniqueness_concurrent():
    ...
    assert len(unique_ids) == len(all_event_ids), f"Found {len(all_event_ids) - len(unique_ids)} duplicate IDs"
...
def test_event_id_streaming_scenario():
    ...
    events.append(Event(
        ...
        content=types.Content(
            role='user',
            parts=[types.Part(text='Please call a tool and inform me before running it')]
        )
    ))
    ...
    assert len(unique_ids) == len(event_ids), f"Found {len(event_ids) - len(unique_ids)} duplicate IDs in streaming scenario"

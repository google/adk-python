...
@pytest.mark.asyncio
async def test_request_confirmation_processor_ignores_tampered_original_call():
  """Test that tampered confirmation state does not resume another tool."""
  agent = LlmAgent(name="test_agent", tools=[mock_tool])
  invocation_context = await testing_utils.create_invocation_context(
      agent=agent
  )
  ...
  confirmation_event = invocation_context.session.events[1]
  confirmation_function_call = confirmation_event.get_function_calls()[0]
  confirmation_function_call.args["originalFunctionCall"] = {
      "id": MOCK_FUNCTION_CALL_ID,
      "name": "delete_user_account",
      "args": {"user_id": "victim_user"},
  }

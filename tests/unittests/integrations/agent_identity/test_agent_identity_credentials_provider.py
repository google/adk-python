...
def test_get_client_uses_rest_transport(mock_client_class):
    provider = _agent_identity_credentials_provider._AgentIdentityCredentialsProvider()
...
async def test_get_auth_credential_raises_error_if_upstream_call_fails(
    ...
    mock_client.retrieve_credentials.side_effect = Exception("API Quota Exhausted")

...
from google.adk.tools.mcp_tool.mcp_session_manager import EnvTransformCallback
...
class TestMCPToolsetEnv:
    ...
    def test_init_with_env_callback(self, mock_stdio_params, sample_env_transform_callback):
        """Test MCPToolset initialization with environment callback."""
        with patch('google.adk.tools.mcp_tool.mcp_toolset.MCPSessionManager') as mock_session_manager:
            toolset = MCPToolset(
            ...
            assert toolset._env_transform_callback == sample_env_transform_callback
    ...
    def test_init_without_env_callback(self, mock_stdio_params):
        ...
        with patch('google.adk.tools.mcp_tool.mcp_toolset.MCPSessionManager') as mock_session_manager:
            ...
            mock_session_manager.assert_called_once_with(
                connection_params=mock_stdio_params,
                errlog=toolset._errlog,
                env_transform_callback=None
            )
            
            assert toolset._env_transform_callback is None
    ...
    async def test_get_tools_passes_context_to_session_manager(
        ...
        with patch('google.adk.tools.mcp_tool.mcp_toolset.MCPSessionManager') as mock_session_manager_class:
            ...
            mock_session_manager.create_session.assert_called_once_with(mock_readonly_context)
    ...
    def test_both_auth_and_env_callbacks(self, mock_stdio_params):
        ...
        with patch('google.adk.tools.mcp_tool.mcp_toolset.MCPSessionManager') as mock_session_manager:
            toolset = MCPToolset(
                ...
                env_transform_callback=env_callback
            )
            
            # Verify both callbacks are stored
            assert toolset._auth_transform_callback == auth_callback
            assert toolset._env_transform_callback == env_callback

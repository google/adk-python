...
def mock_mcp_imports():
    ...
    with patch.dict('sys.modules', {
        ...
        with patch('google.adk.tools.mcp_tool.mcp_session_manager.StdioServerParameters', mock_stdio_params), \
             patch('google.adk.tools.mcp_tool.mcp_session_manager.ClientSession', mock_client_session):
            yield


# Import after mocking to avoid MCP dependency issues
from google.adk.tools.mcp_tool.mcp_session_manager import MCPSessionManager, EnvTransformCallback
...
@pytest.fixture
def sample_env_transform_callback():
    """Create a sample environment transform callback."""
...
class TestMCPSessionManagerEnv:
    ...
    def test_init_with_env_callback(self, mock_stdio_params, sample_env_transform_callback):
        """Test MCPSessionManager initialization with environment callback."""
        session_manager = MCPSessionManager(
            connection_params=mock_stdio_params,
            env_transform_callback=sample_env_transform_callback
        )
        
        assert session_manager._env_transform_callback == sample_env_transform_callback
    ...
    def test_init_without_env_callback(self, mock_stdio_params):
        ...
        assert session_manager._env_transform_callback is None
        assert session_manager._connection_params == mock_stdio_params

    def test_extract_env_from_context_with_callback(
        self, mock_stdio_params, sample_env_transform_callback, mock_readonly_context
    ):
    ...
    def test_extract_env_from_context_no_context(
        self, mock_stdio_params, sample_env_transform_callback
    ):
    ...
    def test_extract_env_from_context_callback_exception(
        ...
        session_manager = MCPSessionManager(
            connection_params=mock_stdio_params,
            env_transform_callback=failing_callback
        )
    ...
    def test_inject_env_vars_stdio_params(self, mock_stdio_params):
        ...
        with patch('google.adk.tools.mcp_tool.mcp_session_manager.StdioServerParameters') as mock_stdio_class:
            ...
            mock_stdio_class.assert_called_once_with(
                ...
                env={'EXISTING_VAR': 'existing_value', 'API_KEY': 'test_key', 'NEW_VAR': 'new_value'},
                cwd=None,
                encoding=None,
    ...
    def test_inject_env_vars_non_stdio_params(self, mock_sse_params):
        ...
        """
        session_manager = MCPSessionManager(
            connection_params=mock_sse_params
        )
    ...
    async def test_create_session_with_env_injection(
        ...
        with patch('google.adk.tools.mcp_tool.mcp_session_manager.AsyncExitStack', return_value=mock_exit_stack), \
             patch('google.adk.tools.mcp_tool.mcp_session_manager.stdio_client', return_value=mock_client), \
             patch('google.adk.tools.mcp_tool.mcp_session_manager.ClientSession', return_value=mock_session), \
             patch('google.adk.tools.mcp_tool.mcp_session_manager.isinstance') as mock_isinstance:
            ...
            def isinstance_side_effect(obj, cls):
                ...
                if obj == mock_stdio_params:
                    ...
                    if hasattr(cls, '__name__') and cls.__name__ == 'StdioServerParameters':
                        return True
                    elif 'StdioServerParameters' in str(cls):
                        return True
                    elif hasattr(cls, '__name__') and cls.__name__ in ['SseServerParams', 'StreamableHTTPServerParams']:
                        return False
                    elif any(name in str(cls) for name in ['SseServerParams', 'StreamableHTTPServerParams']):
                        return False
                ...
                try:
                    original_result = isinstance.__wrapped__(obj, cls) if hasattr(isinstance, '__wrapped__') else type(obj) is cls
                    return original_result
                except:
                    return False
            
            mock_isinstance.side_effect = isinstance_side_effect
            mock_exit_stack.enter_async_context.side_effect = [mock_transports, mock_session]

...
def mock_mcp_imports():
    ...
    with patch.dict('sys.modules', {
        ...
        with patch('google.adk.tools.mcp_tool.mcp_tool.McpBaseTool', mock_tool_class):
            yield
...
class TestMCPToolAuth:
    ...
    def test_init_with_auth(self, mock_mcp_tool, mock_session_manager, mock_auth_scheme, mock_auth_credential):
    ...
    @pytest.mark.asyncio
    async def test_run_async_with_auth_logging(self, mock_mcp_tool, mock_session_manager, 
                                               mock_auth_scheme, mock_auth_credential, caplog):
        ...
        with caplog.at_level(logging.INFO):
            result = await tool.run_async(args={"test": "value"}, tool_context=mock_tool_context)
        
        # Verify the tool was called
        mock_session.call_tool.assert_called_once_with("test_tool", arguments={"test": "value"})
    ...
    @pytest.mark.asyncio
    async def test_run_async_without_auth_no_logging(self, mock_mcp_tool, mock_session_manager, caplog):

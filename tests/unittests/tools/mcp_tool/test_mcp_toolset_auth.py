...
def mock_readonly_context_no_auth():
  ...
  context = MagicMock(spec=ReadonlyContext)
  context.state = MappingProxyType({"other_data": "test_value"})
  return context
...
def mock_readonly_context_custom_auth():
  ...
  context = MagicMock(spec=ReadonlyContext)
  context.state = MappingProxyType({
      "custom_auth": {"type": "bearer", "token": "custom_token_123"},
      "other_data": "test_value",
  })
  return context
...
class TestMCPToolsetAuth:
  ...
  def test_extract_auth_direct_extraction(
      self,
    ...
    with patch("google.adk.tools.mcp_tool.mcp_toolset.MCPSessionManager"):
      ...
      auth_scheme, auth_credential = toolset._extract_auth_from_context(
          mock_readonly_context_with_auth
      )
  ...
  def test_extract_auth_no_auth_in_context(
      self, mock_connection_params, mock_readonly_context_no_auth
  ):
    ...
    with patch("google.adk.tools.mcp_tool.mcp_toolset.MCPSessionManager"):
      ...
      auth_scheme, auth_credential = toolset._extract_auth_from_context(
          mock_readonly_context_no_auth
      )

      assert auth_scheme is None
      assert auth_credential is None

  def test_extract_auth_with_callback(
      self,
      mock_connection_params,
      mock_readonly_context_custom_auth,
      mock_auth_scheme,
      mock_auth_credential,
  ):
  ...
  def test_extract_auth_callback_fallback(
      self,
      mock_connection_params,
      mock_readonly_context_with_auth,
      mock_auth_scheme,
  ...
  @pytest.mark.asyncio
  async def test_get_tools_no_auth_in_context(
      self, mock_connection_params, mock_readonly_context_no_auth
  ):
  ...
  def test_extract_auth_invalid_types(self, mock_connection_params):
    ...
    context.state = MappingProxyType({
        ...
        "auth_credential": {
            "invalid": "credential"
        },  # Should be AuthCredential instance
    })

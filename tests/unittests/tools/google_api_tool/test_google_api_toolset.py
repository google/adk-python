...
@pytest.fixture
def mock_google_api_tool_instance(mock_rest_api_tool): # Renamed from mock_google_api_tool
...
class TestGoogleApiToolset:
  ...
  def test_init(
      self, mock_converter_class, mock_openapi_toolset_class, mock_converter_instance, mock_openapi_toolset_instance
  ):
  ...
  async def test_get_tools(
    ...
    mock_openapi_toolset_instance.get_tools.assert_called_once_with(mock_readonly_context)
  ...
  def test_configure_auth(
    ...
    tool_set = GoogleApiToolset(api_name=TEST_API_NAME, api_version=TEST_API_VERSION)
    client_id = "test_client_id"
    client_secret = "test_client_secret"
  ...
  def test_set_tool_filter(
    ...
    def new_filter_predicate(tool_name: str, tool: RestApiTool, readonly_context: Optional[ReadonlyContext] = None) -> bool:
        return True

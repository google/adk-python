...
# ── tool_filter vs system instruction consistency (#6448) ──


def test_filtered_system_instruction_omits_script_and_resource_usage():
  """Filtered builder must not advertise script/resource tools as usable."""
  instruction = skill_toolset._build_skill_system_instruction(
      allowed_tools={"list_skills", "load_skill"}
  )
  assert "Use `run_skill_script` to run scripts" not in instruction
  assert "The `load_skill_resource` tool is for viewing" not in instruction
...
async def test_process_llm_request_with_prefix_and_tool_filter(
  ...
  assert "`my_load_skill`" in instruction
  assert "Use `my_run_skill_script` to run scripts" not in instruction
  assert "The `my_load_skill_resource` tool is for viewing" not in instruction

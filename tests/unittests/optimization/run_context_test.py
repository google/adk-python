...
class TestBuiltInsRejectContextInContractsPr:
  ...
  async def test_simple_prompt_optimizer_rejects_context(self):
    ...
    with mock.patch(
      ...
      optimizer = SimplePromptOptimizer(SimplePromptOptimizerConfig())
    sampler = mock.MagicMock(spec=Sampler)
    with pytest.raises(UnsupportedOptimizationContextError):
      await optimizer.optimize(
          mock.MagicMock(), sampler, run_context=OptimizationRunContext()
      )

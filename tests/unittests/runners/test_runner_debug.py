...
class TestRunDebug:
  ...
  async def test_run_debug_single_query(self):
    ...
    with mock.patch.object(runner, "run_async", side_effect=mock_run_async):
      ...
      events = await runner.run_debug(
          "Hello, how are you?", print_output=False, return_events=True
      )
      ...
      session = await runner.session_service.get_session(
          app_name=runner.app_name, user_id="default", session_id="default"
      )
  ...
  @pytest.mark.asyncio
  async def test_run_debug_empty_query(self):
    """Test run_debug with no queries (session setup only)."""
    agent = Agent(
        name="test_agent",
        model="gemini-2.5-flash-lite",
    ...
    events = await runner.run_debug(
        None, print_output=False, return_events=True
    )

    # With no queries, returns empty list when return_events=True
    assert events == []
  ...
  @pytest.mark.asyncio
  async def test_run_debug_with_return_events(self):
    """Test that return_events=True returns events, False returns None."""
    agent = Agent(
        name="test_agent",
        model="gemini-2.5-flash-lite",
        instruction="Test agent.",
    ...
    with mock.patch.object(runner, "run_async", side_effect=mock_run_async):
      # Test with return_events=True
      events = await runner.run_debug(
          "Query", print_output=False, return_events=True
      )
      assert events is not None
      assert len(events) == 1

      # Test with return_events=False (default)
      events = await runner.run_debug("Query", print_output=False)
      assert events is None

  @pytest.mark.asyncio
  async def test_run_debug_without_print_output(self, capsys):
    """Test that print_output=False suppresses printing."""
    agent = Agent(
        name="test_agent",
        model="gemini-2.5-flash-lite",
        instruction="Test agent.",
    ...
    with mock.patch.object(runner, "run_async", side_effect=mock_run_async):
      # Execute with print_output=False
      await runner.run_debug("Test query", print_output=False)
  ...
  @pytest.mark.asyncio
  async def test_run_debug_custom_session_name(self):
    """Test run_debug with custom session_name."""
    agent = Agent(
        name="test_agent",
        model="gemini-2.5-flash-lite",
        instruction="Test agent.",
    ...
    with mock.patch.object(runner, "run_async", side_effect=mock_run_async):
      # Execute with custom session name
      await runner.run_debug(
          "Query", session_name="custom_debug_session", print_output=False
      )
  ...
  async def test_run_debug_custom_user_id(self):
    ...
    with mock.patch.object(runner, "run_async", side_effect=mock_run_async):
      ...
      await runner.run_debug(
          "Query", user_id="test_user_123", print_output=False
      )
  ...
  async def test_run_debug_with_run_config(self):
    ...
    with mock.patch.object(
      ...
      await runner.run_debug(
          "Query", run_config=custom_config, print_output=False
      )
  ...
  async def test_run_debug_session_persistence(self):
    ...
    with mock.patch.object(runner, "run_async", side_effect=mock_run_async):
      ...
      events1 = await runner.run_debug(
          "First message", print_output=False, return_events=True
      )
      assert events1[0].content.parts[0].text == "First response"

      # Second call to same session
      events2 = await runner.run_debug(
          "Second message", print_output=False, return_events=True
      )
      assert (
  ...
  @pytest.mark.asyncio
  async def test_run_debug_error_handling(self):
    ...
    # Create a mock session service that fails then returns None
    mock_session_service = mock.AsyncMock(spec=InMemorySessionService)
    mock_session_service.create_session.side_effect = AlreadyExistsError(
        "Already exists"
    )
    ...
    # Should raise ValueError when session can't be created or retrieved
    with pytest.raises(ValueError) as exc_info:
      await runner.run_debug("Test query", print_output=False)

    assert "Failed to create or retrieve session" in str(exc_info.value)
  ...
  async def test_run_debug_with_existing_session(self):
    ...
    with mock.patch.object(runner, "run_async", side_effect=mock_run_async):
      # Execute with same session name (should retrieve existing)
      events = await runner.run_debug(
          "Query",
  ...
  async def test_run_debug_with_tool_calls(self, capsys):
    ...
    with mock.patch.object(runner, "run_async", side_effect=mock_run_async):
      # Execute with print_output=True and verbose=True to see tool calls
      events = await runner.run_debug(
          "Calculate 5 + 3", print_output=True, return_events=True, verbose=True
      )
  ...
  async def test_run_debug_with_executable_code(self, capsys):
    ...
    with mock.patch.object(runner, "run_async", side_effect=mock_run_async):
      events = await runner.run_debug(
          "Run some code", print_output=True, return_events=True, verbose=True
      )
  ...
  async def test_run_debug_with_inline_data(self, capsys):
    ...
    with mock.patch.object(runner, "run_async", side_effect=mock_run_async):
      events = await runner.run_debug(
          "Show image", print_output=True, return_events=True, verbose=True
      )
  ...
  async def test_run_debug_with_file_data(self, capsys):
    ...
    with mock.patch.object(runner, "run_async", side_effect=mock_run_async):
      events = await runner.run_debug(
          "Reference file", print_output=True, return_events=True, verbose=True
      )
  ...
  async def test_run_debug_with_mixed_parts(self, capsys):
    ...
    with mock.patch.object(runner, "run_async", side_effect=mock_run_async):
      events = await runner.run_debug(
          "Mixed response", print_output=True, return_events=True, verbose=True
      )
  ...
  async def test_run_debug_with_long_output_truncation(self, capsys):
    ...
    with mock.patch.object(runner, "run_async", side_effect=mock_run_async):
      events = await runner.run_debug(
          "Process data", print_output=True, return_events=True, verbose=True
      )
  ...
  async def test_run_debug_with_empty_parts_list(self, capsys):
    ...
    with mock.patch.object(runner, "run_async", side_effect=mock_run_async):
      events = await runner.run_debug(
          "Test query", print_output=True, return_events=True
      )

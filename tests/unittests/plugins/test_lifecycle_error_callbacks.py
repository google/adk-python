...
class ObservabilityPlugin(BasePlugin):
    ...
    async def on_agent_error_callback(
        ...
        if self._raise_on_error:
            raise RuntimeError(f"{self.name} intentionally raised in on_agent_error_callback")

    async def on_run_error_callback(
        self, *, invocation_context, error
    ) -> None:
        self.run_error_calls.append(
            {"invocation_context": invocation_context, "error": error}
        )
        if self._raise_on_error:
            raise RuntimeError(f"{self.name} intentionally raised in on_run_error_callback")

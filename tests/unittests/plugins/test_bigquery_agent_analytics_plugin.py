...
#   C1       attributes.adk.node {path, run_id, parent_path}
...
class TestAdkEnvelope:
  ...
  async def test_envelope_on_event_row(
    ...
    # C1: node defaults to path="" with parent_path=null (no synthesis).
    assert adk["node"]["path"] == ""
    assert adk["node"]["parent_path"] is None

  @pytest.mark.asyncio
  async def test_envelope_node_with_parent_path(
      self,
      bq_plugin_inst,
      mock_write_client,
      invocation_context,
    ...
    """C1: parent_path is everything before the final '/segment@run_id'."""
    event = event_lib.Event(
        author="agent_b",
    ...
    assert adk["node"]["parent_path"] == "wf/A@1"

...
async def main():
  ...
  config = LocalAgentConfig(
      ...
      policies=[
          ...
          # Confine the built-in file tools (view_file, write, etc.) to the
          # checked-out workspace. CapabilitiesConfig() enables all built-in
          # tools, and the run_command policy above does not cover view_file,
          # whose reader has no path confinement. Without this, an instruction
          # injected via the untrusted PR/issue text this agent processes could
          # read files outside the workspace (e.g. the credentials file pointed
          # to by GOOGLE_APPLICATION_CREDENTIALS, or /proc/self/environ) and
          # surface their contents in the public report the agent posts.
          *policy.workspace_only([os.getcwd()]),
      ],

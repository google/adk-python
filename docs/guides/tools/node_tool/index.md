...
An `LlmAgent` natively accepts any `Workflow` or `BaseNode` instance in its `tools` list. The node retains its native identity in `agent.tools`, and the framework automatically resolves it into a tool declaration and execution adapter when invoking the model.
...
The tool adapter reads the input schema of the node to generate a function declaration for the model. Upon receiving arguments from the model, the framework validates the input dictionary against the node input schema before invoking the node runtime.
...
The agent accepts any instance derived from `BaseNode`, including `Workflow` graphs and functions decorated with `@node`. Passing an agent directly as a tool is rejected because agents operate with conversational session semantics and belong in `sub_agents`.
...
Passing a function decorated with `@node` directly to an agent `tools` argument exposes it as a tool without requiring manual adapter wrappers. Tool parameter names, types, and descriptions are inferred directly from the function signature and docstrings.
